...
{else}
    ...
    <div id="blockpadding">
        <form action="{$ADMIN_URL}/docs/edit/" method="post" name="newsform" enctype="multipart/form-data">
            <div class="rowform"><label>Заголовок/Название:</label><input type="text" name="title" value="{$_val.title}"></div>
            <div class="rowform"><label>Файл:<br /><em>Условия файла: файл должен быть в формате doc, docx, xls, xlsx, txt, rtf, pdf. Максимальный размер файла - 5Gb. Если выберите файл, старый автоматически удалится.</em></label><input type="file" name="docs"></div>

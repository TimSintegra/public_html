...
if (isset($_POST['save_sorting']) && $_POST['save_sorting'] == 'Сохранить сортировку') {
    if (count($_POST['msort']) > 0) {
        foreach ($_POST['msort'] as $keySort => $valueSort) {
            $db->exec("UPDATE advantages SET msort='" . intval($valueSort) . "' WHERE id='" . intval($keySort) . "'");
        }
    }
}

$errorData = '';
$dataList = $db->getAll("SELECT * FROM advantages ORDER BY msort");
$numData = count($dataList);
if ($dataList) {
    $tableData = array();
    $tableAttr = 'width="100%" border="0"';
    $trAttr = array ('class="eventr"', 'class="oddtr"');
    $tableHeader = '<a href="#">Преимущество</a><i class="strelka">&rarr;</i>'
        . '<i class="place">Всего преимуществ:</i><i class="nn">' . $numData . '</i>';
    $thAttr = array(
        'width="50"',
        '',
        '',
    ...
    foreach ($dataList as $_data) {
        ...
        $tdAttr[] = 'align="center"';
        if ($_data['image'] != "") {
            $_image = '<a href="' . _ADMIN_URL . '/../content/img/main/' . $_data['image']
                . '" class="lightbox"><img src="' . _ADMIN_URL . '/../content/img/main/'

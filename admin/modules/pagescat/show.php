...
if (isset($_POST['save_sorting']) && $_POST['save_sorting'] == 'Сохранить сортировку') {
    if (count($_POST['msort']) > 0) {
        foreach ($_POST['msort'] as $keySort => $valueSort) {
            $db->exec("UPDATE pagescats SET msort='" . intval($valueSort) . "' WHERE id='" . intval($keySort) . "'");
        }
    }
}
$errorData = '';
...
if ($dataList) {
    ...
    $tableHeader = '<a href="#">Категории страниц</a><i class="strelka">&rarr;</i>'
        . '<i class="place">Всего категорий:</i><i class="nn">' . $numData . '</i>';
    $thAttr = array(
        'width="50"',
        '',
        '',
    ...
    foreach ($dataList as $_data) {
        ...
        $url = _HTTP_URL . '/pagescats/' . $_data['url'] . '.html';
        ...
        if (isset($_data['main_text']) && $_data['main_text'] != '') {
            if ($tdStyle == 1)
                $tableData[] = '<a href="#" onclick="showUrlInDialog(\'' . _ADMIN_URL . '/ext/pagecatdetails/'

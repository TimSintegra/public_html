...
if (isset($_POST['save_sorting']) && $_POST['save_sorting'] == 'Сохранить сортировку') {
    if (count($_POST['msort']) > 0) {
        foreach ($_POST['msort'] as $keySort => $valueSort) {
            $db->exec("UPDATE services SET msort='" . intval($valueSort) . "' WHERE id='" . intval($keySort) . "'");
        }
    }
}
if (isset($id) && $id != '' && $id != 0) {
    ...
    $subTitle = '<i class="strelka">&rarr;</i><i class="place">Категория "'
        . $db->getOne("SELECT title FROM servicescats WHERE id='" . $id . "'") . '"</i>';
} else {
    $where = "";
    $subTitle = "";
}
$errorData = '';
$dataList = $db->query("SELECT * FROM services " . $where . "ORDER BY msort");
$numData = $dataList->rowCount();
if ($dataList) {
    $tableData = array();
    $tableAttr = 'width="100%" border="0"';
    $trAttr = array ('class="eventr"', 'class="oddtr"');
    $tableHeader = '<a href="#">Услуги</a>' . $subTitle . '<i class="strelka">&rarr;</i>'
    ...
    $colsTitles = "№,Категория,Заголовок,Изображение,Ссылка,На главной,Основной текст,Сортировка,Действия";
    $no = 1;
    foreach ($dataList as $_data) {
        ...
        $category = $db->getOne("SELECT title FROM servicescats WHERE id='" . $_data['cid'] . "'");
        $tdAttr[] = 'align="center"';
        if ($tdStyle == 1)
        ...
        $tdAttr[] = 'align="center"';
        if ($_data['image'] != "") {
            $_image = '<a href="' . _ADMIN_URL . '/../content/img/pages/' . $_data['image']
                . '" class="lightbox"><img src="' . _ADMIN_URL . '/../content/img/pages/'
        ...
        $tdAttr[] = 'align="center"';
        $url = _HTTP_URL . '/services/' . $_data['url'] . '.html';
        if ($tdStyle == 1)
            $tableData[] = '<s>' . $url . '</s>';
        else
            $tableData[] = $url;
        ...
        if (isset($_data['main_text']) && $_data['main_text'] != '') {
            if ($tdStyle == 1)
                $tableData[] = '<a href="#" onclick="showUrlInDialog(\'' . _ADMIN_URL . '/ext/servicesdetails/'

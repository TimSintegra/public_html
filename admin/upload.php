...
$main_receiver 	= 'kraftnk@gmail.com';				//Who receive the email when files get uploaded
...
$max_file_size	= isset($_REQUEST['ax-max-file-size']) && !empty($_REQUEST['ax-max-file-size']) ?$_REQUEST['ax-max-file-size']:'1000M';
...
if($currByte==0)
{
	...
	{
		echo json_encode(array('name'=>$file_name, 'size'=>$full_size, 'status'=>-1, 'info'=>'Достигнут максимальный размер файлы: '.$max_file_size));
		return false;
	}
	...
	if(!$file_name)
	{
		echo json_encode(array('name'=>$tmp_fn, 'size'=>$full_size, 'status'=>-1,'info'=>'Название файла запрещено.'));
		return false;
	}
	...
	{
		echo json_encode(array('name'=>$file_name, 'size'=>$full_size, 'status'=>-1,'info'=>'Данный тип файла запрещен'));
		return false;
	}
}
...
if($is_ajax)//Ajax Upload, FormData Upload and FF3.6 php://input upload
{
    ...
    if($isLast!='true')
    {
    	echo json_encode(array('name'=>basename($full_path), 'size'=>$full_size, 'status'=>1, 'info'=>'Часть загружена'));
    }
}
else //Normal html and flash upload
{
	...
	{
		echo json_encode( array('name'=>basename($full_path), 'size'=>$full_size, 'status'=>-1, 'info'=>'Ошибка переноса файла') );
		return  false;
	}
}
...
if($isLast == 'true')
{
	...
	echo json_encode(array('name'=>basename($full_path), 'size'=>$full_size, 'status'=>1, 'info'=>'Загружено'));
	@success($full_path);
}